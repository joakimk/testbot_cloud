...
describe TestbotCloud::Server::Bootstrap do
  ...
  it "should be able to upload bootstrap files and run them" do
    ...
    bootstrap.should_receive(:system).with("scp -r bootstrap -o StrictHostKeyChecking=no ubuntu@15.14.13.12:~ &> /dev/null").and_return(true)
  ...
  it "should not try to bootstrap if the connection fails" do
    ...
    bootstrap.should_not_receive(:system).with("scp -r bootstrap -o StrictHostKeyChecking=no ubuntu@15.14.13.12:~ &> /dev/null")
